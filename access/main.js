...
// Settings page logic
$('.menulist.sos:nth-child(7)').on('click', function() {
  $('.menulid').fadeOut(1);
  $('.settingspage').fadeIn(1);
  $('html').css('background', '#C9D8E7');

  // Populate form with current values
  $('.settings-input[data-field="fullname"]').val(fullname);
  $('.settings-input[data-field="name"]').val(name);
  $('.settings-input[data-field="birthdate"]').val(birthdate);
  $('.settings-input[data-field="passport_id"]').val(passport_id);
  $('.settings-input[data-field="kpp_id"]').val(kpp_id);
  $('.settings-input[data-field="rights_valid_until"]').val(rights_valid_until);
  $('.settings-input[data-field="rights_categories"]').val(rights_categories);
  $('.settings-input[data-field="rights_tsc"]').val(rights_tsc);
  $('.settings-input[data-field="rights_id"]').val(rights_id);
  $('.settings-input[data-field="covid_valid_until"]').val(covid_valid_until);
  $('.settings-input[data-field="covid_certificate_id"]').val(covid_certificate_id);
  $('.settings-toggle[data-field="isRightsEnabled"]').prop('checked', isRightsEnabled);
  $('.settings-toggle[data-field="isCovidCertificateEnabled"]').prop('checked', isCovidCertificateEnabled);

  // Load image previews
  try {
    var imgData = JSON.parse(localStorage.getItem('diya_images') || '{}');
    if(imgData.photo) $('.settings-photo-preview[data-preview="photo"]').css('background-image', 'url(' + imgData.photo + ')');
    if(imgData.rights) $('.settings-photo-preview[data-preview="rights"]').css('background-image', 'url(' + imgData.rights + ')');
    if(imgData.sign) $('.settings-photo-preview[data-preview="sign"]').css('background-image', 'url(' + imgData.sign + ')');
  } catch(e) {}
});

$('.settings-back').on('click', function() {
  $('.settingspage').fadeOut(1);
  $('.menulid').fadeIn(1);
  $('.icon').removeClass('act');
  $('.icon.men').addClass('act');
  $('html').css('background', '#C9D8E7');
...
$('.settings-file-btn').on('click', function() {
  $(this).siblings('.settings-file-input').click();
});
...
$('.settings-file-input').on('change', function() {
  ...
  var preview = $('.settings-photo-preview[data-preview="' + key + '"]');
  var reader = new FileReader();
  reader.onload = function(e) {
    preview.css('background-image', 'url(' + e.target.result + ')');
    preview.data('new-image', e.target.result);
  };
...
$('#settingsSaveBtn').on('click', function() {
  var settings = {
    fullname: $('.settings-input[data-field="fullname"]').val(),
    name: $('.settings-input[data-field="name"]').val(),
    birthdate: $('.settings-input[data-field="birthdate"]').val(),
    passport_id: $('.settings-input[data-field="passport_id"]').val(),
    kpp_id: $('.settings-input[data-field="kpp_id"]').val(),
    rights_valid_until: $('.settings-input[data-field="rights_valid_until"]').val(),
    rights_categories: $('.settings-input[data-field="rights_categories"]').val(),
    rights_tsc: $('.settings-input[data-field="rights_tsc"]').val(),
    rights_id: $('.settings-input[data-field="rights_id"]').val(),
    covid_valid_until: $('.settings-input[data-field="covid_valid_until"]').val(),
    covid_certificate_id: $('.settings-input[data-field="covid_certificate_id"]').val(),
    isRightsEnabled: $('.settings-toggle[data-field="isRightsEnabled"]').is(':checked'),
    isCovidCertificateEnabled: $('.settings-toggle[data-field="isCovidCertificateEnabled"]').is(':checked')
  };
  localStorage.setItem('diya_settings', JSON.stringify(settings));

  // Save images
  var imgData = {};
  try { imgData = JSON.parse(localStorage.getItem('diya_images') || '{}'); } catch(e) {}
  $('.settings-photo-preview').each(function() {
    var data = $(this).data('new-image');
    if(data) imgData[$(this).data('preview')] = data;
  });
  localStorage.setItem('diya_images', JSON.stringify(imgData));

  // Show saved feedback and reload
  var btn = $(this);
  btn.text('Збережено!');
